...
This repo has all the source code for a technical talk on testing. 

In this repository all solutions are set up to use Aspire. Aspire is an orchestrator for the entire application and will take care of configuring dependencies, building, and running the application. The resources that make up the application are defined in `apphost.cs` including application code and external dependencies.

## Project structure
Every demo will be in its own folder called "Demo # (description)". Each demo will be its own solution. The following rules apply:
* Use as little code as possible to show case the concepts of the demo
* Put source code for the app in a "src" folder
* Put the test code in a "tests" folder
* Use central package managemen
* Use xUnit and Microsoft Testing Platform V2 for testing

## Creating new projects
Use the `dotnet new`command for creating new API:s, testprojects, Aspire Apphosts etc to get all relevant files created from the templates. Then review the created files so the comply with the rules of this repo.
